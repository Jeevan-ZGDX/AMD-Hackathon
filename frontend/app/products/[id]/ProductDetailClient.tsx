...
export default function ProductDetailClient({ product, isOutOfStock }: { product: any, isOutOfStock: boolean }) {
  ...
  return (
    <div ref={containerRef} className="grid grid-cols-1 lg:grid-cols-2 gap-16">
      ...
      <div ref={imageRef} className="space-y-4">
        <div className="aspect-square bg-slate-100 rounded-3xl overflow-hidden border border-slate-100 shadow-inner group">
          <img 
      ...
      <div ref={contentRef} className="flex flex-col">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="flex items-center gap-2 bg-emerald-50 px-3 py-1 rounded-full border border-emerald-100 hover:bg-emerald-100 transition-colors cursor-pointer">
        ...
        <h1 className="text-4xl font-black text-slate-900 mb-4 leading-tight hover:text-blue-600 transition-colors cursor-pointer">
          {product.name}
        </h1>
        ...
        <div className="flex items-center gap-4 mb-8 pb-8 border-b border-slate-100">
          <div className="flex items-center gap-1 group cursor-pointer">
            {[...Array(5)].map((_, i) => (
              <Star 
                key={i} 
                className={`w-4 h-4 transition-transform group-hover:scale-110 ${i < Math.floor(product.rating) ? 'text-yellow-400 fill-yellow-400' : 'text-slate-200'}`} 
                style={{ transitionDelay: `${i * 50}ms` }}
              />
            ))}
          </div>
          <span className="text-sm text-slate-500 font-bold">{product.rating} Rating</span>
          <span className="text-sm text-slate-400 font-medium hover:text-blue-500 cursor-pointer transition-colors">| {product.review_count} verified reviews</span>
        </div>
        ...
        <div className="flex flex-col gap-6 mb-12">
           <div className="flex items-center gap-4">
              ...
              {isOutOfStock ? (
                <span className="text-red-600 font-black text-sm uppercase animate-pulse">Out of Stock</span>
              ) : (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
            <Link 
              href="/"
              className="btn-secondary !py-5 flex items-center justify-center text-sm uppercase tracking-widest font-black hover:-translate-y-1 transition-transform duration-300 shadow-sm hover:shadow-md"
            >
              Back to Marketplace
            </Link>
          </div>
        ...
        <div className="grid grid-cols-3 gap-4 mt-12 pt-12 border-t border-slate-100">
           <div className="flex flex-col items-center gap-2 text-center group cursor-default">
              <div className="p-3 bg-blue-50 rounded-full group-hover:bg-blue-100 transition-colors group-hover:scale-110 duration-300">
